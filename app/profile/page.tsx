...
type IconName =
  ...
  | "truck" | "check" | "return" | "wallet" | "pin" | "heart"
  | "ticket" | "star" | "headset" | "info" | "globe" | "logout";
...
function Icon({ name, size = 20 }: { name: IconName; size?: number }) {
  const paths: Record<IconName, React.ReactNode> = {
    ...
    phone: <path d="M6.6 3.5 9 3l1.5 4-2 1.4a15.5 15.5 0 0 0 5.1 5.1l1.4-2 4 1.5-.5 2.4a2 2 0 0 1-2.2 1.6A16.5 16.5 0 0 1 4.9 5.7a2 2 0 0 1 1.7-2.2Z" />,
    chevron: <path d="m9 6 6 6-6 6" />,
    package: <><path d="m21 8-9 5-9-5 9-5 9 5Z" /><path d="M3 8v8l9 5 9-5V8M12 13v8" /></>,
    truck: <><path d="M3 6h11v10H3zM14 10h4l3 3v3h-7z" /><circle cx="7" cy="18" r="2" /><circle cx="18" cy="18" r="2" /></>,
    check: <><path d="M5 12 10 17 20 7" /><path d="M3 12a9 9 0 1 0 18 0" /></>,
    return: <><path d="M9 7 4 12l5 5" /><path d="M4 12h10a5 5 0 0 1 5 5v1" /></>,
    wallet: <><path d="M3 6.5A2.5 2.5 0 0 1 5.5 4H19v16H5.5A2.5 2.5 0 0 1 3 17.5v-11Z" /><path d="M3 7h16v4h-4a2 2 0 0 0 0 4h4v5M15 13h.01" /></>,
  ...
  return <svg aria-hidden="true" className="profile-icon" width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">{paths[name]}</svg>;
}

const accountOptions: { title: string; subtitle: string; icon: IconName; action: string }[] = [
  { title: "My Orders", subtitle: "View, track and manage your orders", icon: "package", action: "orders" },
  { title: "My Wallet", subtitle: "View balance & payment methods", icon: "wallet", action: "wallet" },
  { title: "My Addresses", subtitle: "Manage your delivery addresses", icon: "pin", action: "addresses" },
  { title: "Wishlist", subtitle: "Your saved items", icon: "heart", action: "wishlist" },
  { title: "Coupons & Offers", subtitle: "Save more with exclusive deals", icon: "ticket", action: "deals" },
  { title: "Reviews & Ratings", subtitle: "Your feedback matters", icon: "star", action: "reviews" },
  { title: "Help & Support", subtitle: "Get help or contact us", icon: "headset", action: "support" },
  { title: "About Us", subtitle: "Learn more about our app", icon: "info", action: "about" },
...
export default function ProfilePage() {
  ...
  const handleAction = (action: string) => {
    ...
    else setMessage(action === "language" ? "Language selection is ready." : `${action[0].toUpperCase()}${action.slice(1)} selected.`);
  };
  ...
  return (
    <>
      ...
      <section className="profile-section"><h2>Account</h2><div className="account-options-card">{accountOptions.map((option) => <button className="account-option" type="button" key={option.title} onClick={() => handleAction(option.action)}><span className="account-option-icon"><Icon name={option.icon} size={20} /></span><span className="account-option-copy"><strong>{option.title}</strong><small>{option.subtitle}</small></span><Icon name="chevron" size={19} /></button>)}</div></section>

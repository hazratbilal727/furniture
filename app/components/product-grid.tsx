...
export function ProductGrid({
  ...
  useEffect(() => {
    const syncWishlist = () => {
      try {
        setWishlist(
          JSON.parse(localStorage.getItem("muntazir-wishlist") ?? "[]"),
        );
      } catch {
        setWishlist([]);
      }
    ...
    return () => {
      ...
      window.removeEventListener("muntazir-wishlist-updated", syncWishlist);
    };
  }, []);
  const visible = useMemo(
    () =>
      products
        .filter(
          (product) =>
            (!onlyDeals || product.originalPrice) &&
    ...
    [category, onlyDeals, query, sort],
  );

  const addToCart = (product: Product) => {
    const stored = JSON.parse(
      localStorage.getItem("muntazir-cart") ?? "[]",
    ) as Array<Product & { quantity: number }>;
    const next = stored.some((item) => item.id === product.id)
      ? stored.map((item) =>
          item.id === product.id
            ? { ...item, quantity: item.quantity + 1 }
            : item,
        )
      : [...stored, { ...product, quantity: 1 }];
  ...
  const openDetails = (product: Product, position: number) => {
    if ((onlyDeals && position < 8) || (!onlyDeals && product.id <= 8))
      setSelectedProduct(product);
    else
      setMessage(
        "Detailed views are currently available for the first eight pieces.",
      );
  };
  ...
  return (
    <>
          ...
          <div className="shop-search-panel">
            <label className="search-field">
              ...
              <input
                ...
                aria-label="Search furniture by name, room, or style"
              />
              {query && (
                <button
                  type="button"
      ...
      <div className="filter-row route-filters">
        {categoryOptions.map((item) => (
          ...
          >
            {item === "All" ? "All pieces" : item}
          </button>
        ))}
        <span className="product-count">{visible.length} pieces</span>
      </div>
        ...
        {visible.map((product, position) => (
          <article className="product-card" key={product.id}>
            ...
            <div className="product-info">
              ...
              <h3>{product.name}</h3>
              <p className="route-product-copy">{product.description}</p>
              <strong>
                {money(product.price)}{" "}
            ...
            <div className="product-actions">
              <button
                ...
                onClick={() => openDetails(product, position)}
              >
                Details
              </button>
              <button className="add-button" onClick={() => addToCart(product)}>
                Add to cart <i className="icon fa-solid fa-plus" />
              </button>
            </div>
          </article>
        ))}
      ...
      {cart.length > 0 && (
        <aside className="cart-summary" id="cart">
          <div>
            <p className="eyebrow">YOUR BAG</p>
            <h3>
              {cart.reduce((total, item) => total + item.quantity, 0)}{" "}
              {cart.length === 1 ? "piece" : "pieces"} selected
            </h3>
          </div>
          <strong>
            {money(
          ...
          <button
            ...
            onClick={() =>
              setMessage("Checkout is available through our studio team.")
            }
          >
            Checkout <i className="icon fa-solid fa-arrow-right" />
          </button>
        </aside>
      )}
      {selectedProduct && (
        ...
        >
          ...
          >
            ...
            <div className="detail-content">
              ...
              <h2 id="shop-product-detail-title">{selectedProduct.name}</h2>
              <strong>
                {money(selectedProduct.price)}{" "}
                {selectedProduct.originalPrice && (
                  <del>{money(selectedProduct.originalPrice)}</del>
                )}
              </strong>
              ...
              <div className="detail-actions">
                ...
                <span className="detail-note">
                  <i className="icon fa-solid fa-truck" /> Delivery across
                  Pakistan
                </span>
              </div>
            </div>
